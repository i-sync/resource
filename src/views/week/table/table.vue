<template>
  <div class="app-container calendar-list-container">
    ...
    <el-table :key='tableKey' :data="list" v-loading="listLoading" element-loading-text="给我一点时间" border fit highlight-current-row style="width: 100%">
      ...
      <el-table-column min-width="300px" label="本周工作情况">
        <template scope="scope">
      ...
      <el-table-column width="80px" label="互相评价">
        <template scope="scope">
          <icon-svg v-for="n in +scope.row.importance" icon-class="star" class="meta-item__icon" :key="n"></icon-svg>
        </template>
      </el-table-column>

      <el-table-column align="center" label="上级评语" width="95">
        <template scope="scope">
          <span class="link-type" @click='handleFetchPv(scope.row.pageviews)'>{{scope.row.pageviews}}</span>
        </template>
      </el-table-column>

      <el-table-column class-name="status-col" label="工作状态" width="90">
        <template scope="scope">
